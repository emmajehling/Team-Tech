...
def h(j):           #inserts appropriate h value for each circumferential line
    if j==0:
        ...
        return 66.609375
def k(i,g,b):         #material properties
    if i in range (0,g):
        return 0.45          #milk k-value
    elif i in range (g,b):
        return 1.05            #glass k-value
def c(i,g,b):           #material properties
    if i in range (0,g):
        return 3770             #milk c-value
    elif i in range (g,b):
        return 800             #glass c-value
def p(i,g,b):           #material properties
    if i in range (0,g):
        return 1030                 #milk p-value
    elif i in range (g,b):
        return 2500             #glass p-value

def generalnodal(i,j,m,data,ct,t,g,b): #i=1-8, j=1-3 (milk AND glass nodes not on boundry)
...
def innerring(i,j,m,data,ct,t,g,b):    #i=0, j=1-3
...
def outerring(i,j,m,data,ct,t,g,b): #i=9, i=1-3
...
def leadingcirc(i,j,m,data,ct,t,g,b): #i=1-8, j=0 (milk AND glass nodes on leading boundry)
...
def endingcirc(i,j,m,data,ct,t,g,b): #i=1-8, j=4 (milk AND glass nodes on ending boundry)
...
def innermostleadingcirc(i,j,m,data,ct,t,g,b): #i=0,j=0
...
def outermostleadingcirc(i,j,m,data,ct,t,g,b): #i=9, j=0
...
def innermostendingcirc(i,j,m,data,ct,t,g,b): #i=0,j=4
...
def outermostendingcirc(i,j,m,data,ct,t,g,b): #i=9,j=4
...
def runmodel(data,ct,t,g,b):    #b is one more than the last node (for k, c, p functions)
    m=0
...
def main():
    ...
    finishedmodel=runmodel(blankmatrix,ct,t,g,i)
